...
class Caller:
    ...
    If the current process belongs to the LLM's processes,
    it will launch the LLM and wait for requests.
    """
    ...
    def __init__(
        ...
        llm_processes = list(
            range(
                ...
                config.distributed_setup_args.n_rl_processes
                + config.distributed_setup_args.n_llm_processes,
            )
        )

        lamorel_logger.info(
            "Init rl group for process {}".format(accelerator.process_index)
        )
        self._rl_group = dist.new_group(ranks=rl_processes, backend="gloo")
        lamorel_logger.info(
            "Init llm group for process {}".format(accelerator.process_index)
        )
        self._llm_group = dist.new_group(ranks=llm_processes, backend="gloo")
        lamorel_logger.info(
            "Init rl-llm group for process {}".format(accelerator.process_index)
        )
        self._llm_master_process = (
            rl_processes[-1] + 1
        )  # First LLM process is considered as master
        self._rl_llm_group = dist.new_group(
            ranks=rl_processes + [self._llm_master_process], backend="gloo"
        )
    ...
    def score(
        ...
        candidates: typing.List[typing.List[str]],
        additional_module_function_keys: typing.List[str] = [],
        **kwargs
    ):
        module_function_keys = ["__score"]
        module_function_keys.extend(additional_module_function_keys)
        result = self.__call_model(
            InstructionsEnum.FORWARD,
            True,
    ...
    def generate(self, contexts: list, **kwargs):
        return self.__call_model(
            InstructionsEnum.GENERATE, True, contexts=contexts, **kwargs
        )
    ...
    def custom_module_fns(
        self,
        module_function_keys: typing.List[str],
        contexts: typing.List[str],
        **kwargs
    ):
        return self.__call_model(
    ...
    def __call_model(self, instruction, expect_answer, **kwargs):
        dist.gather_object(
            obj={"instruction": instruction, **kwargs},
            object_gather_list=None,
            dst=self._llm_master_process,
        ...
        if expect_answer:
            results = [
                None
                for _ in range(dist.get_world_size(group=self._rl_llm_group))
            ]
            dist.broadcast_object_list(
                object_list=results,
                src=self._llm_master_process,
                group=self._rl_llm_group,
            )
