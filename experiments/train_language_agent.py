...
def reward_function_shapped(
    ...
    if reward > 0:
        return [
            20 * reward - np.log(subgoal_proba / policy_value),
            -np.log(subgoal_proba / policy_value),
        ]
    else:
        return [
            0 - np.log(subgoal_proba / policy_value),
            0 - np.log(subgoal_proba / policy_value),
        ]
...
class LogScoringModuleFn(BaseModuleFunction):
    ...
    def forward(self, forward_outputs, minibatch, tokenized_contexts, **kwargs):
        ...
        ):  # hence input should be removed from result
            logits = forward_outputs["logits"][
                :, len(tokenized_contexts["input_ids"]) - 1 : -1, :
            ]
            output_tokens = minibatch["input_ids"][
                :, len(tokenized_contexts["input_ids"]) :
            ]
        else:
            logits = forward_outputs["logits"][
                :, :-1, :
            ]  # skip </s> token appended by tokenizer
            output_tokens = minibatch["decoder_input_ids"][
                :, 1:
            ]  # skip pad token
        ...
        mask = torch.ones(
            tokens_logprobs.shape, dtype=torch.bool, device=self.device
        )
        for i, _output in enumerate(output_tokens):
            for j, _token in enumerate(_output):
                if _token != self._pad_token:
                    mask[i, j] = False
        masked_token_probs = tokens_logprobs.masked_fill(
            mask, 1.0
        )  # apply mask
...
class ValueHeadModuleFn(BaseModuleFunction):
    ...
    def initialize(self):
        llm_hidden_size = self.llm_config.to_dict()[
            self.llm_config.attribute_map["hidden_size"]
        ]
    ...
    def forward(self, forward_outputs, minibatch, tokenized_contexts, **kwargs):
        if self._model_type == "causal":
            model_head = forward_outputs["hidden_states"][-1][
                :, len(tokenized_contexts["input_ids"]) - 1, :
            ]
        else:
...
class ActionHeadsModuleFn(BaseModuleFunction):
    ...
    def forward(self, forward_outputs, minibatch, tokenized_contexts, **kwargs):
        ...
        if self._model_type == "causal":
            model_head = forward_outputs["hidden_states"][-1][
                0, len(tokenized_contexts["input_ids"]) - 1, :
            ]
        else:
            model_head = forward_outputs["decoder_hidden_states"][-1][:, 0, :]
...
class PPOUpdater(BaseUpdater):
    ...
    def perform_update(
        self, contexts, candidates, _current_batch_ids, **kwargs
    ):
        # Initialize model if asked + optimizer
        if (
            not hasattr(self, "optimizer")
            and "load_fine_tuned_version" not in kwargs
        ):
            self.optimizer = torch.optim.Adam(
                self._llm_module.parameters(),
                kwargs["lr"],
        ...
        if (
            "load_embedding" in kwargs
            or "load_fine_tuned_version" in kwargs
            or "save_first_last" in kwargs
        ):
            # If asked, only do embedding weights loading
            if (
                "load_embedding" in kwargs
                and kwargs["load_embedding"]
                and not hasattr(self, "is_embedding_loaded")
            ):
                pretrained_weights = torch.load(
                    kwargs["llm_path"] + "/pytorch_model.bin"
                )
                state_dict = OrderedDict(
                    {
                        k: v
                        for k, v in pretrained_weights.items()
                        if "embed" in k or "shared" in k
                        # Warning: this may fail if the model shares other things than embedding weights
                    }
                )
                self._llm_module.module._LLM_model.load_state_dict(
                    state_dict, strict=False
                )
                self.is_embedding_loaded = True

                torch.save(
                    self._llm_module.state_dict(),
                    kwargs["saving_path_model"]
                    + "/"
                    + kwargs["id_expe"]
                ...
                "load_fine_tuned_version" in kwargs
                and kwargs["load_fine_tuned_version"]
                ...
                "save_first_last" in kwargs
                and kwargs["save_first_last"]
            ...
            surr2 = (
                torch.clamp(
                    ratio, 1.0 - kwargs["clip_eps"], 1.0 + kwargs["clip_eps"]
                )
                * sb["advantage"]
            )
            ...
            value_clipped = sb["value"] + torch.clamp(
                values - sb["value"], -kwargs["clip_eps"], kwargs["clip_eps"]
            )
            ...
            grad_norm = (
                sum(
                    p.grad.data.detach().cpu().norm(2) ** 2
                    for p in self._llm_module.parameters()
                    if p.grad is not None
                )
                ** 0.5
            )
            torch.nn.utils.clip_grad_norm_(
                self._llm_module.parameters(), kwargs["max_grad_norm"]
            )
            self.optimizer.step()
...
def run_agent(args, algo, id_expe):
    ...
    while status["num_frames"] < args.num_steps:
        ...
        reshaped_return_per_episode = utils.synthesize(
            logs["reshaped_return_per_episode"]
        )
        reshaped_return_bonus_per_episode = utils.synthesize(
            logs["reshaped_return_bonus_per_episode"]
        )
...
def main(config_args):
    ...
    if not config_args.lamorel_args.llm_args.pretrained:
        id_expe += "load_embedding_{}_".format(
            config_args.rl_script_args.load_embedding
        )

    if config_args.rl_script_args.use_action_heads:
        id_expe += "use_action_heads_{}_".format(
            config_args.rl_script_args.use_action_heads
        )

    if config_args.rl_script_args.nbr_obs != 3:
        id_expe += "nbr_obs_{}_".format(config_args.rl_script_args.nbr_obs)

    id_expe += "nbr_actions_{}_".format(
        len(config_args.rl_script_args.action_space)
    )
    ...
    id_expe += "shape_reward_beta_{}_".format(
        config_args.rl_script_args.reward_shaping_beta
    ) + "seed_{}".format(config_args.rl_script_args.seed)

    model_path = os.path.join(
        config_args.rl_script_args.saving_path_model, id_expe
    )
    ...
    if config_args.lamorel_args.distributed_setup_args.n_llm_processes > 0:
        if os.path.exists(
            ...
            if (
                not config_args.lamorel_args.llm_args.pretrained
                and config_args.rl_script_args.load_embedding
            ):
                lm_server.update(
                    [
                        None
        ...
        algo = LLMPPOAgent(
            ...
            config_args.rl_script_args.discount,
            config_args.rl_script_args.lr,
            config_args.rl_script_args.beta1,
            config_args.rl_script_args.beta2,
            config_args.rl_script_args.gae_lambda,
            config_args.rl_script_args.entropy_coef,
            config_args.rl_script_args.value_loss_coef,
            config_args.rl_script_args.max_grad_norm,
            config_args.rl_script_args.adam_eps,
            config_args.rl_script_args.clip_eps,
            config_args.rl_script_args.epochs,
            ...
            number_envs,
            subgoals,
            config_args.rl_script_args.nbr_obs,
            id_expe,
        ...
        algo = DRRNAgent(
            ...
            subgoals,
            reshape_reward,
            config_args.rl_script_args.spm_path,
            max_steps=number_envs * 4,
            saving_path=config_args.rl_script_args.saving_path_model
            + "/"
            + id_expe,
            save_frequency=1,
        )
